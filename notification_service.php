...
// Mail helper (PHPMailer fallback)
...
function update_order_status($conn, $order_id, $new_status, $admin_id, $notes = '') {
    ...
    // If status moved to Out for Delivery, generate delivery OTP (pending send)
    if ($new_status === 'Out for Delivery') {
        // Only generate if none exists for this order
        $check_sql = "SELECT id, status FROM delivery_otps WHERE order_id = ? LIMIT 1";
        $ch = mysqli_prepare($conn, $check_sql);
        if ($ch) {
            mysqli_stmt_bind_param($ch, 'i', $order_id);
            mysqli_stmt_execute($ch);
            $resch = mysqli_stmt_get_result($ch);
            $exists = mysqli_fetch_assoc($resch);
        ...
        if (!$exists) {
            // Generate OTP record (do not send yet)
            generate_delivery_otp($conn, $order_id, $user_id);
        }
    }
...
// ======================================
// 8. Delivery OTP Functions
// ======================================

/**
 * Generate a secure 6-digit OTP for an order and store it (encrypted + hashed).
 * Returns true on success (does not expose OTP), or array with plaintext when
 * $return_plaintext is true (used only internally when sending immediately).
 */
function generate_delivery_otp($conn, $order_id, $user_id, $expiry_minutes = null, $return_plaintext = false) {
    $expiry_minutes = $expiry_minutes ?? (defined('OTP_EXPIRY_MINUTES') ? OTP_EXPIRY_MINUTES : 30);
    ...
    // Hash for verification and encrypted storage for resend
    $otp_hash = password_hash($otp, PASSWORD_DEFAULT);

    // Encrypt using OpenSSL with OTP_ENC_KEY
    $key = defined('OTP_ENC_KEY') ? OTP_ENC_KEY : 'please_change_this_to_a_secure_random_key';
    $iv = substr(hash('sha256', uniqid('', true)), 0, 16);
    $ciphertext = base64_encode($iv . openssl_encrypt($otp, 'AES-256-CBC', $key, OPENSSL_RAW_DATA, $iv));

    $generated_at = date('Y-m-d H:i:s');
    $expires_at = date('Y-m-d H:i:s', strtotime("+{$expiry_minutes} minutes"));

    // Insert delivery_otp record
    $sql = "INSERT INTO delivery_otps (order_id, user_id, otp_encrypted, otp_hash, generated_at, expires_at, status, attempts, max_attempts) VALUES (?, ?, ?, ?, ?, ?, 'pending', 0, 5)";
    $stmt = mysqli_prepare($conn, $sql);
    if (!$stmt) {
        error_log('Failed preparing delivery_otp insert: ' . mysqli_error($conn));
        return false;
    }
    mysqli_stmt_bind_param($stmt, 'iissss', $order_id, $user_id, $ciphertext, $otp_hash, $generated_at, $expires_at);
    $ok = mysqli_stmt_execute($stmt);
    if (!$ok) {
        error_log('Failed executing delivery_otp insert: ' . mysqli_stmt_error($stmt));
        mysqli_stmt_close($stmt);
        return false;
    }
    ...
    // Create a notification instructing the user to choose delivery OTP method
    create_notification($conn, $user_id, $order_id, 'delivery_otp_pending', 'Delivery OTP Pending', 'Your order is out for delivery. Please choose how you would like to receive the delivery OTP.', "orders_new.php?order_id={$order_id}");

    // Return plaintext only when requested by the immediate sender (internal use)
    if ($return_plaintext) {
        return ['otp_id' => $otp_id, 'otp' => $otp];
    }
...
/**
 * Send delivery OTP for a given otp_id via selected method (email,sms,whatsapp)
 * Marks status as 'sent' and records method.
 */
function send_delivery_otp($conn, $otp_id, $method = 'email') {
    // Fetch OTP record
    $sql = "SELECT * FROM delivery_otps WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);
    ...
    if (!$otp_row) return false;

    // Decrypt OTP
    $key = defined('OTP_ENC_KEY') ? OTP_ENC_KEY : 'please_change_this_to_a_secure_random_key';
    $raw = base64_decode($otp_row['otp_encrypted']);
    $iv = substr($raw, 0, 16);
    $cipherdata = substr($raw, 16);
    $otp_plain = openssl_decrypt($cipherdata, 'AES-256-CBC', $key, OPENSSL_RAW_DATA, $iv);

    // Fetch user contact. Some schemas may use either phone_number or phone.
    $contact_check = mysqli_query($conn, "SHOW COLUMNS FROM users LIKE 'phone_number'");
    $has_phone_number = $contact_check && mysqli_num_rows($contact_check) > 0;
    mysqli_free_result($contact_check);

    if ($has_phone_number) {
        $user_sql = "SELECT email, phone_number AS phone_number FROM users WHERE id = ?";
    } else {
        $user_sql = "SELECT email, phone AS phone_number FROM users WHERE id = ?";
    }

    $ustmt = mysqli_prepare($conn, $user_sql);
    if ($ustmt) {
        mysqli_stmt_bind_param($ustmt, 'i', $otp_row['user_id']);
        mysqli_stmt_execute($ustmt);
        $u_res = mysqli_stmt_get_result($ustmt);
    ...
    if ($method === 'email' && !empty($user['email'])) {
        // Send by email using helper
        $subject = "Your delivery OTP for order #{$otp_row['order_id']}";
        $body = "<p>Your one-time delivery verification code is: <strong>{$otp_plain}</strong>.</p><p>It expires at {$otp_row['expires_at']}.</p>";
        $sent = send_email_smtp($user['email'], $subject, $body);
    } elseif ($method === 'sms' && !empty($user['phone_number'])) {
        // Send via SMS helper (simulated locally)
        $msg = "Your Bazario delivery OTP is: {$otp_plain}. Expires at {$otp_row['expires_at']}";
        $sent = send_sms_whatsapp($user['phone_number'], $msg, 'sms');
    } elseif ($method === 'whatsapp' && !empty($user['phone_number'])) {
        $msg = "Your Bazario delivery OTP is: {$otp_plain}. Expires at {$otp_row['expires_at']}";
        $sent = send_sms_whatsapp($user['phone_number'], $msg, 'whatsapp');
    } else {
        // Unsupported or missing contact
        return false;
    }

    if ($sent) {
        $update_sql = "UPDATE delivery_otps SET method = ?, status = 'sent' WHERE id = ?";
        $ust = mysqli_prepare($conn, $update_sql);
        mysqli_stmt_bind_param($ust, 'si', $method, $otp_id);
        mysqli_stmt_execute($ust);
        mysqli_stmt_close($ust);

        create_notification($conn, $otp_row['user_id'], $otp_row['order_id'], 'delivery_otp_sent', 'Delivery OTP Sent', 'Your delivery OTP has been sent via ' . strtoupper($method) . '.', "orders_new.php?order_id={$otp_row['order_id']}");
        return true;
    }
...
/**
 * Verify an entered OTP for an order. Returns array with success boolean and message.
 */
function verify_delivery_otp($conn, $order_id, $entered_otp, $ip_address = null) {
    // Load OTP row for the order
    $sql = "SELECT * FROM delivery_otps WHERE order_id = ?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, 'i', $order_id);
    mysqli_stmt_execute($stmt);
    $res = mysqli_stmt_get_result($stmt);
    ...
    if ($otp_row['expires_at'] < $now) {
        // Mark expired
        $upd = mysqli_prepare($conn, "UPDATE delivery_otps SET status = 'expired' WHERE id = ?");
        mysqli_stmt_bind_param($upd, 'i', $otp_row['id']);
        mysqli_stmt_execute($upd);
        mysqli_stmt_close($upd);

        // Log
        $log_stmt = mysqli_prepare($conn, "INSERT INTO otp_verification_logs (otp_id, order_id, user_id, attempt_time, ip_address, success, note) VALUES (?, ?, ?, NOW(), ?, 0, 'expired')");
        mysqli_stmt_bind_param($log_stmt, 'iiis', $otp_row['id'], $order_id, $otp_row['user_id'], $ip_address);
    ...
    $verified = password_verify($entered_otp, $otp_row['otp_hash']);

    // Log attempt
    $log_stmt = mysqli_prepare($conn, "INSERT INTO otp_verification_logs (otp_id, order_id, user_id, attempt_time, ip_address, success, note) VALUES (?, ?, ?, NOW(), ?, ?, ?)");
    $note = $verified ? 'verified' : 'invalid';
    $success_int = $verified ? 1 : 0;
    mysqli_stmt_bind_param($log_stmt, 'iiiisi', $otp_row['id'], $order_id, $otp_row['user_id'], $ip_address, $success_int, $note);
    mysqli_stmt_execute($log_stmt);
    mysqli_stmt_close($log_stmt);

    // Increment attempts
    $inc = mysqli_prepare($conn, "UPDATE delivery_otps SET attempts = attempts + 1 WHERE id = ?");
    mysqli_stmt_bind_param($inc, 'i', $otp_row['id']);
    mysqli_stmt_execute($inc);
    mysqli_stmt_close($inc);

    if ($verified) {
        // Mark OTP verified and order delivered
        $ver = mysqli_prepare($conn, "UPDATE delivery_otps SET status = 'verified', verified_at = NOW() WHERE id = ?");
        mysqli_stmt_bind_param($ver, 'i', $otp_row['id']);
        mysqli_stmt_execute($ver);
        mysqli_stmt_close($ver);

        // Mark order as Delivered and set delivered_at
        $upd_order = mysqli_prepare($conn, "UPDATE orders SET status = 'Delivered', delivered_at = NOW(), updated_at = NOW() WHERE id = ?");
        mysqli_stmt_bind_param($upd_order, 'i', $order_id);
        mysqli_stmt_execute($upd_order);
        mysqli_stmt_close($upd_order);

        // Notifications
        create_notification($conn, $otp_row['user_id'], $order_id, 'delivered', 'Order Delivered', 'Your order has been verified and marked as delivered.', "orders_new.php?order_id={$order_id}");
        // Notify admin(s) - simplistic approach: log activity
        log_activity($conn, $otp_row['user_id'], 'DELIVERY_VERIFIED', "Order #$order_id delivery verified via OTP");

        // TODO: update inventory/statistics as needed elsewhere

        return ['success' => true, 'message' => 'OTP verified. Order marked as delivered'];
    }
